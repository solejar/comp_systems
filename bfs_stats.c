    ...
    int file_size = 1; //this is for testing the simple code
        ...
        if(pid !=0){
            ...
            while((i<kids)&&(pid!=0)){
                ...
                int this_elems = elems_left/(kids+1);

                int elems_left -= this_elems;
                pid = fork();
                if(pid !=0){
                    //int * parent_stats = stats()

                    int * child_stats = malloc(this_elems*sizeof(int));

                    read(istream_parent,child_stats,this_elems*sizeof(int));
