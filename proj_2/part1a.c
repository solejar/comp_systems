...
int *iterative_spawn(int kids, int data_length, int * vals){
    ...
    for(int i = 0;i<kids;i++){
        ...
        if(pid ==0){//if it's the child
            ...
            //send my vals as signals

            value.sival_int = my_stats[0]; //this is the sum
            sigqueue(getppid(),SIGRTMIN,value);

            value.sival_int = my_stats[1]; //this is the min
            sigqueue(getppid(),SIGRTMIN+1,value);

            value.sival_int = my_stats[2]; //this is the max
    ...
    //initialize count of vals received
    int sum_c = 0;
    int min_c = 0;
    int max_c = 0;
    ...
    for (int i = 0;i<3*kids;i++){ 
        ...
        if (sigwaitinfo(&mask, &info) == -1) {
            ...
            if(num==SIGRTMIN){//a sum was recieved
              all_child_stats[0][sum_c] = val;
              sum_c++;
            }else if(num==SIGRTMIN+1){//a min was received
              all_child_stats[1][min_c] = val;
              min_c++;
            }else if(num==SIGRTMIN+2){//a max was received
    ...
    //this condition is true when all values are read in as expected
    if(sum_c == kids && min_c==kids &&max_c==kids){
        ...
        printf("Not all data from %d kids received. Got %d sums, %d mins, %d maxs\n",kids, sum_c,min_c,max_c);
    }
...
int main(int argc, char *argv[]){
    ...
    //let's make a mask to block IPC signals, so they can be queued
    sigemptyset(&mask);
    ...
    //set disposition of IPC signals, so we can extract info from them
